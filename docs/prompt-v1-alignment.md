...
- 启动时可以为空
- 用户第一次把任务发给当前发送目标后，第一张卡片入场
- 后续同一可继续会话会追加新 turn，强制新会话时新增卡片
...
- 已在 WSL 中真实探测到
- 已通过 ACP 接入真实任务执行链路
- Hermes 过程事件进入会话卡片，是 `Stage 1` 异构成立的关键
...
3. 打磨 Claude / Hermes 双入口下的当前发送目标与多会话体验
