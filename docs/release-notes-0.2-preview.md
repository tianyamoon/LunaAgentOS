...
0.3 keeps strengthening that carrying capacity: make Agent entries understandable, model management truthful, health diagnostics trustworthy, and Runtime Session distinct from future Task. Handoff and automatic orchestration are outside 0.3.
...
- **Clearer multi-session surface**: Session Card headers, status counts, session titles, and action groups are reorganized so current work and historical sessions stay easier to read.
