import { isAuthoritativeMemorySession } from "./sessionStatusSource.js";
...
export function projectSessionListItems({
  ...
  const archivedIds = new Set(archivedSessions.map((item) => item?.id).filter(Boolean));
  const liveItems = sessions
    .filter((session) => isAuthoritativeMemorySession(session, {
      hasPersistedHistory: archivedIds.has(session?.id),
      isSessionActive,
    }))
    .map((session) => {
